...
public class OptimizerUtils
{
	...
	public static double getLeftIndexingSparsity( long rlen1, long clen1, double nnz1, long rlen2, long clen2, double nnz2 )
	{
		...
		if( nnz1>=0 && scalarRhs )
			...
			lnnz = nnz1 + (double) rlen2*clen2; // nnz(left) + nnz(right_dense)
		lnnz = Math.min(lnnz,(double)  rlen1*clen1);

		return getSparsity(rlen1, clen1, (lnnz>=0) ? lnnz : (double) rlen1*clen1);
	}
	...
	public static double getOuterNonZeros(long n1, long n2, long nnz1, long nnz2, OpOp2 op) {
		...
		switch(op) {
			...
			case OR:
				return (double) n1 * n2
					-  ((double)n1-nnz1) * (n2-nnz2);
			case MULT:
			case AND:
				return (double) nnz1 * nnz2;
			default:
				return (double) n1 * n2;
		}
	}
	...
	public static double getSparsity( long dim1, long dim2, double nnz ) {
