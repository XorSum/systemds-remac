...
public class RewriteAlgebraicSimplificationStatic extends HopRewriteRule {
    ...
    private void rule_AlgebraicSimplification(Hop hop, boolean descendFirst) {
        if (hop.isVisited())
            return;
        //   System.out.println("Before Algebraic Simplification");
        ...
        for (int i = 0; i < hop.getInput().size(); i++) {
            ...
            //hi = mySimplifyRule(hop, hi, i);  // t(y)%*%t(x) -> t(x%*%y)
            // hi = mySimplifyRule2(hop, hi, i);
...
//        System.out.println("After Algebraic Simplification");
//        System.out.println(Explain.explain(hop));
            ...
            if (HopRewriteUtils.isDataGenOp(left, OpOpDG.RAND) &&
                ...
                if (HopRewriteUtils.isBinary(bop, OpOp2.MULT, OpOp2.PLUS, OpOp2.MINUS, OpOp2.DIV)
                    ...
                    switch (bop.getOp()) { //fuse via scale and shift
                        ...
                        case PLUS:
                        case MINUS:
                            gen = HopRewriteUtils.copyDataGenOp(inputGen,
                                    1, sval * ((bop.getOp() == OpOp2.MINUS) ? -1 : 1));
                            break;
                        case DIV:
                            ...
                            if (HopRewriteUtils.isMatrixMultiply(ef)) {
                                ...
                                if (HopRewriteUtils.isTransposeOperation(c) &&
                                    ...
                                    if (a == td && a == tf && b == tc && b == te) {
                                        ...
                                        Hop result = HopRewriteUtils.createBinary(ab_tab, tab_b, OpOp2.DIV);
